...
func RateLimit(cfg *Config) gin.HandlerFunc {
	...
	// 验证 Limit 必须大于 0
	if cfg.Limit <= 0 {
		panic("ratelimit middleware requires a positive limit")
	}

	// 验证 Window 必须大于 0
	if cfg.Window <= 0 {
		panic("ratelimit middleware requires a positive window")
	}
	...
	return func(c *gin.Context) {
		...
		// 验证 key 不为空
		if key == "" {
			log.Error(c, "empty key generated by KeyFunc",
				types.Field{Key: "error", Value: "invalid rate limit key"},
			)
			c.AbortWithStatusJSON(http.StatusInternalServerError,
				cfg.Formatter.FormatError(codes.RateLimitError, "Invalid rate limit key"))
			return
		}
		...
		operation := func() (bool, error) {
			...
			allowed, err := cfg.Limiter.AllowN(c, key, 1, cfg.Limit, cfg.Window)
		...
		if cfg.WaitMode {
			// Wait 模式下，直接使用限流器的 Wait，不使用中间件的重试
			ctx, cancel := context.WithTimeout(c.Request.Context(), cfg.WaitTimeout)
			defer cancel()
			...
			// 非 Wait 模式下，使用原有的重试逻辑
			var retryErr error
			allowed, retryErr = doWithRetry(c, key, cfg.Retry, operation)
			duration = time.Since(start)
			LimiterLatency.WithLabelValues(key, "total").Observe(duration.Seconds())

			if retryErr != nil {
				log.Error(c, "rate limit operation failed",
					types.Field{Key: "error", Value: retryErr},
					types.Field{Key: "duration", Value: duration},
				)

				RequestsTotal.WithLabelValues(key, "error").Inc()
				err = errors.NewError(codes.RateLimitError, cfg.Messages.CheckFailedMessage, retryErr)
				c.Error(err)
				c.AbortWithStatusJSON(cfg.Status.CheckFailed,
					cfg.Formatter.FormatError(codes.RateLimitError, cfg.Messages.CheckFailedMessage))
				return
		...
		if !allowed {
			log.Debug(c, "rate limit exceeded",
				types.Field{Key: "duration", Value: duration},
			)
